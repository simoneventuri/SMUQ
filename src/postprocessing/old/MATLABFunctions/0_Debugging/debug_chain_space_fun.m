function i_figure = debug_chain_space_fun(PlottingFormat, i_figure, theta_scale, theta_names, i, j, i_MCMC, i_DR, ...
    ...
    if theta_scale(i,:) == 'log'
        if theta_scale(j,:) == 'log'
            loglog(theta(:,i),theta(:,j),'-k','LineWidth',1.0)
        elseif theta_scale(j,:) == 'lin'
            semilogx(theta(:,i),theta(:,j),'-k','LineWidth',1.0)
        end
    elseif theta_scale(i,:) == 'lin'
        if theta_scale(j,:) == 'log'
            semilogy(theta(:,i),theta(:,j),'-k','LineWidth',1.0)
        elseif theta_scale(j,:) == 'lin'
            plot(theta(:,i),theta(:,j),'-k','LineWidth',1.0)
        end
    end
