...
contains
  ...
  subroutine Draw( This, NbDim, NbTrajectories, Trajectories, StepSize, Indices )
    ...
    allocate(Signs(NbDim,NbTrajectories), stat=StatLoc)
    if ( StatLoc /= 0 ) call Error%Allocate( Name='Signs', ProcName=ProcName, stat=StatLoc )
    Signs = 1
      
    allocate(PermutationIndices(NbDim,NbTrajectories), stat=StatLoc)
    ...
    i = 1
    do i = 1, NbTrajectories
      PermutationIndices(:,i) = LinSequence( SeqStart=1, SeqEnd=NbDim )
      call ScrambleArray( Array=PermutationIndices(:,i), RNG=This%RNG )
      P = Zero

      ii = 1
      do ii = 1, NbDim
        VarR0D = This%RNG%Draw()
        if ( VarR0D < 0.5 ) Signs(ii,i) = - 1
        P(PermutationIndices(ii,i),ii) = One
      end do
      ...
      ii = 1
      do ii = 1, NbDim
        VarR2D(:,ii) = (VarR2D(:,ii)*Signs(ii,i) + One)*VarR0D + XStar(ii,i)
      end do
    ...
    if ( present(StepSize) ) then
      StepSize = PerturbationSize * real(Signs,rkp)
    end if
