  ...
  subroutine AssembleCov( This, Abscissa, Input, Cov, Debug )
    ...
    do i = 1, size(Cov,1)
      ...
      do ii = i, size(Cov,1)
        ...
        Cov(i,ii) = MLoc * dexp(- (dabs(Abscissa(i)-Abscissa(ii))/LLoc)**GamLoc )
    ...
    select type (Input)
      ...
      type is (InputStoch_Type)
        if ( len_trim(This%L_Dependency) /= 0 ) then
          if ( Input%IsStochastic(Label=This%L_Dependency) ) IsStochastic = .true.
        end if
        ...
        if ( len_trim(This%Gam_Dependency) /= 0 ) then
          if ( Input%IsStochastic(Label=This%Gam_Dependency) ) IsStochastic = .true.
        end if
      class default
