...
contains
  ...
  function GetInput(This, Name, Prefix, Directory)
    ...
    SectionName = 'sampler'
    call GetInput%AddSection(Section=SampleMethod_Factory%GetObjectInput(Object=This%Sampler, Name=SectionName,       &
                                                                                       Prefix=PrefixLoc, Directory=DirectorySub))

    if (ExternalFlag) DirectorySub = DirectoryLoc // '/ols_solver'
    SectionName = 'solver'
    call GetInput%AddSection(Section=This%Solver%GetInput(Name=SectionName,Prefix=PrefixLoc, Directory=DirectorySub))
  ...
  subroutine BuildModel(This, Basis, SampleSpace, Responses, Model, IndexSetScheme, Coefficients, Indices, CVErrors,             &
    ...
    do
      ...
      if (size(This%ParamRecord,2) > 0 .and. This%IndexOrder >= IndexMaxOrder) OrderExceededFlag = .true.
