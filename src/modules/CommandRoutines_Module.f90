...
contains
  ...
  subroutine CopyDirectory( Source, Destination, ContentsOnly )
    ...
    call ExecuteSysCommand( CommandLine, Wait=.true., ExitStatus=IOLoc )
    if ( IOLoc /= 0 ) call Error%Raise( Line='Something went wrong copying directory: ' // trim(adjustl(Source)) // ' -> ' //     &
  ...
  subroutine RemoveDirectory( Path, ContentsOnly )
    ...
    call ExecuteSysCommand( CommandLine, Wait=.true., ExitStatus=IOLoc )
    if ( IOLoc /= 0 ) call Error%Raise( Line='Something went wrong removing directory: ' // trim(adjustl(Path)),ProcName=ProcName)
  ...
  subroutine MakeDirectory( Path, Options )
    ...
    if ( IOLoc /= 0 ) call Error%Raise( Line='Something went wrong creating directory: ' // trim(adjustl(Path)),ProcName=ProcName)
  ...
  subroutine RemovePath( Path, Options )
    ...
    call ExecuteSysCommand( CommandLine, ExitStatus=IOLoc )
    if ( IOLoc /= 0 ) call Error%Raise( Line='Something went wrong removing the path: ' // trim(adjustl(Path)),ProcName=ProcName )
  ...
  subroutine ExecuteSysCommand( SysCommand, Wait, ExitStatus ) 
    ...
    if ( IOLoc /= 0 .and. .not. present(ExitStatus) ) call Error%Raise( 'Something went wrong with system command : ' //          &
