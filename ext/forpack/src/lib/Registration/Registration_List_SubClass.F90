SubModule(Registration_Class) Registration_List_SubClass
...
Module Procedure OutputRegisterObjectsListToString
  ...
  integer                                                               ::  i, j, NameLength, CategoryLength
  ...
  do i = 1,This%N
  associate( O => This%Items(i)%O )
    call O%Output( Strings )

    do j = 1,size(Strings)
      call Logger%Write( Strings(j) )
    end do
    call Logger%Write( "************************" )
