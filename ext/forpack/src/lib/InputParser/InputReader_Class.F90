Module InputReader_Class

  use InputSection_Class      ,only:  InputSection_Type
  use String_Library          ,only:  Text_Type
...
!   implicit none
  ...
  Type  ,extends(InputSection_Type) ::  InputReader_Type
    integer                         ::  Unit                                            !< File unit number
    character(:)  ,allocatable      ::  FileName                                        !< Name of the file associated to current object
  contains
    private
    generic ,public ::  Read => ReadInputFromFile, ReadInputFromCharacters, ReadInputFromText                      !< Initializes an Input-Reader
    procedure       ::  ReadInputFromFile
    procedure       ::  ReadInputFromCharacters
    procedure       ::  ReadInputFromText
!     generic     ,public         ::  Output  =>  OutputInputReader                                           !< Outputs an Input object state to a file
!     procedure   ,private        ::  OutputInputReader
!     generic     ,public         ::  Write   =>  WriteInputReaderToUnit
!     procedure   ,private        ::  WriteInputReaderToUnit
  End Type
...
!     End Subroutine
    ...
    Module Subroutine ReadInputFromFile( This, FileName, SectionName, Found, AllowMacro, AllowEnvVar, LogLevel )
      class(InputReader_Type)                               ,intent(out)    ::  This                            !< Passed-object dummy argument
      character(*)                                          ,intent(in)     ::  FileName                        !< Name of the input file to be processed, including path and extention
      character(*)                                ,optional ,intent(in)     ::  SectionName                     !< Name of the main section to be read
      logical                                     ,optional ,intent(out)    ::  Found                           !< Indicator whether the input file has been found. If this argument is not specified and the inpout file is not found, then an error is raised
    ...
    Module Subroutine ReadInputFromCharacters( This, Charac, SectionName, AllowMacro, AllowEnvVar, LogLevel )
      class(InputReader_Type)                               ,intent(out)    ::  This                            !< Passed-object dummy argument
    ...
    Module Subroutine ReadInputFromText( This, Text, SectionName, AllowMacro, AllowEnvVar, LogLevel )
      class(InputReader_Type)                               ,intent(out)    ::  This                            !< Passed-object dummy argument
      type(Text_Type)                                       ,intent(in)     ::  Text                            !< Text object
      character(*)                                ,optional ,intent(in)     ::  SectionName                     !< Name of the main section to be read
      logical                                     ,optional ,intent(in)     ::  AllowMacro                      !< Indicator whether macro substitution is allowed. Default: True
      logical                                     ,optional ,intent(in)     ::  AllowEnvVar                     !< Indicator whether environment variable substitution is allowed. Default: True
...
!     Module Subroutine OutputInputReader( This, Unit )
!       class(InputReader_Type)                               ,intent(in)     ::  This                            !< Passed-object dummy argument
!       integer                                               ,intent(in)     ::  Unit
!     End Subroutine

!     Module Subroutine WriteInputReaderToUnit( This, Unit )
!       class(InputReader_Type)                               ,intent(in)     ::  This                            !< Passed-object dummy argument
!       integer                                               ,intent(in)     ::  Unit
!     End Subroutine
...
! !     Charac  =   Text%GetLines()   !@COMPILER_BUG: gcc-7.3.0: ICE
!     allocate( Charac, source = Text%GetLines() )
! # else
