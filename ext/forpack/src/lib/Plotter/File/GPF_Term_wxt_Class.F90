Module GPF_Term_WXT_Class
  ...
  public  ::  GPF_Term_WXT_Type

  Type  ,extends(GPF_Term_Type) ::  GPF_Term_WXT_Type
    private
    type(GPF_Font_Type)         ::  Font                                            !< Terminal Font object
    character(:)  ,allocatable  ::  Title                                           !< Window Title
    character(:)  ,allocatable  ::  Dashed                                          !< Dashed mode indicator
    character(:)  ,allocatable  ::  Persist                                         !< Persist mode indicator
  contains
    private
    procedure   ,public   ::  Initialize    =>  InitializeTermWXT
    procedure   ,public   ::  Set_Command   =>  SetCommandTermWXT
    procedure   ,public   ::  GetName
  End Type

  Interface

    Module Subroutine InitializeTermWXT( This, Color, FontName, FontSize, Enhanced, Extension, Title, Dashed, Persist, Size, Debug )
      class(GPF_Term_WXT_Type)                              ,intent(inout)  ::  This                            !< Terminal object to be constructed
    ...
    Module Pure Function GetName( This ) result(Name)
      class(GPF_Term_WXT_Type)                              ,intent(in)     ::  This
      character(:)  ,allocatable                                            ::  Name
    End Function

    Module Subroutine SetCommandTermWXT( This )
      class(GPF_Term_WXT_Type)                              ,intent(inout)  ::  This                            !< Passed-object dummy argument corresponding to the Terminal object
    End Subroutine
