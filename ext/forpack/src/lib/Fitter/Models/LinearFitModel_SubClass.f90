SubModule(LinearFitModel_Class) LinearFitModel_SubClass

  implicit none

  character(*)    ,parameter    ::  Model_Name    =   "Linear"
  integer         ,parameter    ::  Model_NParam  =   2
...
Module Procedure Evaluate_0d
  if ( size(Param) < Model_NParam ) then                                                                        !< If the input Param array is too small, then ...
    ...
    y   =     Param(1) + Param(2) * x                                                                           !< ... evaluate the fonction using the input x value and parameters
  end if
End Procedure
...
Pure Module Function Get_LHS_RowVector( This, x ) result(A_Row)
  class(LinearFitModel_Type)                            ,intent(in)     ::  This                              !< Passed-object dummy argument
...
Pure Module Function Get_LHS_RowVector_Jacobian( This, x ) result(dAdx_Row)
  class(LinearFitModel_Type)                            ,intent(in)     ::  This                              !< Passed-object dummy argument
  real(rkp)                                             ,intent(in)     ::  x                                 !> X coordinates
  real(rkp)     ,dimension( This%NParam )                               ::  dAdx_Row                          !> Derivative of a single row of the LHS matrix of the linear system A x = b wrt the x-coordinatereal(rkp)                                                             ::  rdum
  real(rkp)                                                             ::  rdum
