...
!     variables in the last position, Update is called.   Update assumes
...
!     General declarations
  ...
  public       :: STARTUP, Update, REGCF, TOLSET, SING, SS, COV, PARTIAL_CORR, &
                  VMOVE, REORDR, HDIAG, VARPRD, BKSUB2
  private      :: INV

  integer   ,parameter    ,public   ::  rkp   = REAL64
  real(rkp) ,parameter    ,private  ::  Zero  = 0.0_rkp
  real(rkp) ,parameter    ,private  ::  One   = 1.0_rkp
  real(rkp) ,parameter    ,private  ::  Small = 10.0_rkp * tiny(Zero)

  integer                 ,public   ::  NOBS, NCOL, R_DIM
  integer ,allocatable    ,public   ::  VORDER(:), ROW_PTR(:)
  logical                 ,public   ::  INITIALIZED = .False.
  logical                 ,public   ::  TOL_SET     = .False.
  logical                 ,public   ::  RSS_SET     = .False.
  real(rkp) ,allocatable  ,public   ::  D(:), RHS(:), R(:), TOL(:), RSS(:)
  real(rkp)               ,public   ::  SSERR, TOLY
...
subroutine Update( WEIGHT, XROW, YELEM )

  real(rkp)     ,intent(in)     ::  WEIGHT
  real(rkp)     ,intent(in)     ::  YELEM
  real(rkp)     ,intent(inout)  ::  XROW(:)

  integer     ::  I, K, NEXTR
  real(rkp)   ::  W, Y, XI, DI, WXI, DPI, CBAR, SBAR, XK
...
real(rkp), intent(in), optional :: EPS
...
!     Auxiliary routines called: Update, TOLSET
!
!--------------------------------------------------------------------------
...
real(rkp), dimension(NCOL)  :: X, WORK
...
!     element of LINDEP, and use Update to augment the projections in
!     the lower rows of the orthogonalization.
  ...
  if (WORK(COL) <= TEMP) then
    ...
    if (COL < NCOL) then
      ...
      D(COL) = Zero
      RHS(COL) = Zero
      call Update(WEIGHT, X, Y)
                             ! Update automatically increases the number
...
real(rkp), dimension(:), intent(out)  :: COVMAT, STERR

!     Local variables.

integer           :: DIM_RINV, POS, ROW, START, POS2, COL, POS1, K
real(rkp)    :: TOTAL
real(rkp), allocatable, dimension(:)  :: RINV
...
real(rkp), dimension(INVAR+1:NCOL)  :: RMS, WORK
