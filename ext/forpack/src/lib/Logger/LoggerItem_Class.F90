Module LoggerItem_Class
  ...
  use Logger_Parameters ,only:  LogLevel_DEFAULT
  ...
  Type    ::  LoggerItem_Type
    ...
    integer                     ::  LogLevel          =   LogLevel_DEFAULT   ! Log-level associated to current 'LoggerItem' object (Default=Info=3)
    integer                     ::  SavedLogLevel     =   LogLevel_DEFAULT   ! Copie of the Saved value of the 'Mode' (Used to restore)
    integer                     ::  MsgLogLevel       =   LogLevel_DEFAULT   ! Default log-level for all logging message performed by current current 'LoggerItem' object (Default=Info=3)
    integer                     ::  SavedMsgLogLevel  =   LogLevel_DEFAULT   ! Default log-level for all logging message performed by current current 'LoggerItem' object (Default=Info=3)
  contains
    private
...
!   End Interface
  ...
  Interface
    ...
    Pure Module Subroutine SetIndentation( This, Indentation )
      ...
      integer                                     ,optional ,intent(in)     ::  Indentation
    End Subroutine
    Pure Module Function IsLoggerItemActive( This, LogLevel, ActiveProcedures ) result(Active)
      class(LoggerItem_Type)                                ,intent(in)     ::  This                            !< Passed-object dummy argument
      integer                                     ,optional ,intent(in)     ::  LogLevel
      character(:)  ,allocatable ,dimension(:)    ,optional ,intent(in)     ::  ActiveProcedures
      logical                                                               ::  Active
    End Function
  End Interface
